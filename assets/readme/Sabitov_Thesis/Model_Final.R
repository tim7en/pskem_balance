...
# PARAMETERS
K_B <- 0.99#0.989876534 # CN2<-65 NS 0.6561178 original input #changed from 0.924
FCAP <- 0#12.42591 # Field capacity #changed to 0
AM5 <- 0 # Anticident moisture
SAT <- 6.775272384 # Initial saturation zone
K <- 0.45 # coefficient
CN_2 <- 36.05177556
UNSAT <- 0#0.93585302 # changed to 0
...
Data$Glac_Tavg <- (((Data$TMIN - 32) * 5 / 9 - (3643 - 1251) / 1000 * Data$Lapse) + ((Data$TMAX - 32) * 5 / 9 - (3680 - 1251) / 1000 * Data$Lapse)) / 2
...
Data$PET <- Evp(Data, Data$TMAX, Data$TMIN)
...
par(mar = c(5, 6, 4, 1))
# x11()
plot(Data$Q ~ Data$Date, pch = NA_integer_, ylab = "Discharge, cms", xlab = "Date", cex.lab = 2, cex.axis = 1.8)
# main = "Daily streamflow at Pskem river")
lines(Data$Q ~ Data$Date, col = "red", lty = 2, lwd = 2)
lines(y$Flow ~ Data$Date, col = "blue", lty = 1, lwd = 2)
legend("topleft", c("Model", "Actual"), lty = c(2, 1), bty = "n", col = c("red", "blue"), cex = 2)
...
remove(KK, KU, x, A, AM1_D, AM1_G, AM2_G, AM2_D, AM5, CN_1, CN_3, by, CN2, ERR, ERR3, FCAP, i, j, K, k, K_B, loc_0, loc_1, loc_2, loc_3, loc_4, loc_5, loc_6, loc_7, loc_8, loc_9, n, SAT, Fin_EVP, Fin_Ground, Fin_P, Fin_Surf, Fin_Stream, Month_abv, Peak, Average, d, f, limy_1, limy_2, UNSAT, y_dim)
...
plot(monthly_measured$x / 35.31, pch = 17, ylab = "Discharge, cms", xlab = "Month", col = "blue", cex = 1.3, cex.lab = 2, y.intersp = 0.5, ylim = c(0, 200), cex.axis = 1.8)
# main  = "Average monthly streamflow at Pskem River between 2013-2015 water year",
# lines (monthly_measured$x, pch = 17, col = "blue")
points(monthly_mod$x / 35.31, pch = 14, col = "red")
legend("topleft", c("Actual", "Model"), pch = c(17, 14), col = c("blue", "red"), bty = "n", inset = c(0, 0), cex = 2)
# END
par(mar = c(5, 6, 4, 1))
plot((y$Flow / 35.31) ~ Data$Date, pch = NA_integer_, ylab = "Discharge, cms", xlab = "Date", ylim = c(0, 250), cex.axis = 1.6, cex.lab = 2, y.intersp = 0.5)
# main = "Average daily discharge at Pskem River between 2013-2015 water year",
lines(y$Flow / 35.31 ~ Data$Date, col = "blue", lty = 2, bty = "n", lwd = 2)
lines(Data$Q / 35.31 ~ Data$Date, col = "red", lwd = 2)
legend("topleft", lty = c(2, 1), c("Actual", "Model"), cex = 2, col = c("blue", "red"), bty = "n", lwd = c(2, 2))
...
plot(Data$T_av ~ Data$Date, main = "Exponentially-weighted mean of daily air temperatures(2013-2015)", ylim = c(-30, 30), pch = NA_integer_, ylab = "Tair, C", xlab = "Date", cex.main = 1.8, cex.axis = 1.3, cex.lab = 1.3)
# ,
...
plot(Data$PRCP * 25.4 ~ Data$Date, ylim = c(0, 150), main = "Average daily precipitation (2013-2015)", pch = NA_integer_, ylab = "Precip, mm", xlab = "Date", cex.main = 1.8, cex.axis = 1.3, cex.lab = 1.3)
#
lines(Data$Date, Data$PRCP * 25.4, col = "blue", lty = 1)
...
# lines (Data$Date,EMA(Data$PRCP*25.4, n=5, wilder = F), col= "blue", lty = 1)
# lines (lowess(Data$Date,Data$PRCP*25.4,f=.2,iter = 3),col="blue", lty = 2)
...
B <- Bias(Data$Q[which(Data$Date=="2012-10-01"):which(Data$Date=="2015-09-30")]/35.31, Data$Q_obs[which(Data$Date=="2012-10-01"):which(Data$Date=="2015-09-30")]/35.31, length(Data$Q[which(Data$Date=="2012-10-01"):which(Data$Date=="2015-09-30")]))
N <- Nash(Data$Q[which(Data$Date=="2012-10-01"):which(Data$Date=="2015-09-30")]/35.31, Data$Q_obs[which(Data$Date=="2012-10-01"):which(Data$Date=="2015-09-30")]/35.31)

Nash(monthly_mod$x/35.31, monthly_measured$x/35.31)
...
R<-cor (Data$Q/35.31, y$Flow/35.31)
